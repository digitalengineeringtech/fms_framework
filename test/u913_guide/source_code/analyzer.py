...
class ProtocolAnalyzerUI(QMainWindow):
    ...
    def create_rs232_tab(self):
        ...
        self.rs232_channel_combo = QComboBox()
        self.rs232_channel_combo.addItems(["Channel 0", "Channel 1", "Channel 2", "Channel 3"])
        test_layout.addWidget(self.rs232_channel_combo, 2, 1)
        
        select_channel_button = QPushButton("Select Channel")
        select_channel_button.clicked.connect(
            lambda: self.send_command(f"CHANNEL {self.rs232_channel_combo.currentIndex()}")
        )
        ...
        u913_button.clicked.connect(self.run_u913_test)
        ...
        u913_channel_button.clicked.connect(
            lambda: self.run_u913_test_on_channel(u913_channel_combo.currentIndex())
        )
    ...
    def run_rs232_test(self):
        ...
        channel = self.rs232_channel_combo.currentIndex()
        self.send_command(f"CHANNEL {channel}")
        time.sleep(0.2)
