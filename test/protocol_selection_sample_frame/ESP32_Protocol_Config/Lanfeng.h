/**
 * Lanfeng.h - Library for Lanfeng protocol implementation
...
public:
  ...
  static const uint8_t CMD_SET_NOZZLE = 0x02;
  static const uint8_t CMD_START_FLOW = 0x03;
  static const uint8_t CMD_STOP_FLOW = 0x04;
  static const uint8_t CMD_GET_STATUS = 0x05;
  static const uint8_t CMD_SET_PRESSURE = 0x06;
  static const uint8_t CMD_SET_TEMPERATURE = 0x07;
  static const uint8_t CMD_CALIBRATE = 0x08;
  static const uint8_t CMD_EMERGENCY_STOP = 0x09;
  static const uint8_t CMD_RESET = 0x0A;
  ...
  /**
   * Initialize the Lanfeng protocol
...
const uint8_t LanfengProtocol::HEADER[2] = {0xAA, 0x55};
const uint8_t LanfengProtocol::FOOTER[2] = {0x55, 0xAA};
...
bool LanfengProtocol::begin(uint8_t nozzleCount, HardwareSerial &serialPort) {
  _serial = &serialPort;
  ...
  debugPrint("Initializing Lanfeng protocol...");
  
  if (sendCommand(CMD_INIT, data, 1)) {
    if (waitForResponse(CMD_INIT, _timeout)) {
      _isInitialized = true;
      debugPrint("Lanfeng protocol initialized successfully");
      return true;
    } else {
...
bool LanfengProtocol::selectNozzle(uint8_t nozzleIndex) {
  if (!_isInitialized) {
...
bool LanfengProtocol::startFlow(uint8_t flowRate) {
  if (!_isInitialized) {
    _lastError = ERROR_NOT_READY;
    return false;
  }
...
uint8_t LanfengProtocol::getStatus() {
  if (!_isInitialized) {
    _lastError = ERROR_NOT_READY;
    return 0xFF;
  }
...
bool LanfengProtocol::setPressure(uint8_t pressure) {
  if (!_isInitialized) {
    _lastError = ERROR_NOT_READY;
    return false;
  }
...
bool LanfengProtocol::setTemperature(uint8_t temperature) {
  if (!_isInitialized) {
    _lastError = ERROR_NOT_READY;
    return false;
  }
...
bool LanfengProtocol::calibrate() {
  if (!_isInitialized) {
    _lastError = ERROR_NOT_READY;
    return false;
  }
...
bool LanfengProtocol::emergencyStop() {
...
uint8_t LanfengProtocol::getLastError() {
  return _lastError;
}

const char* LanfengProtocol::getErrorMessage(uint8_t errorCode) {
  switch (errorCode) {
    case ERROR_NONE:
      return "No error";
    case ERROR_COMM:
...
void LanfengProtocol::setTimeout(unsigned long timeoutMs) {
  _timeout = timeoutMs;
}

void LanfengProtocol::setDebug(bool enable) {
  _debugEnabled = enable;
}

bool LanfengProtocol::sendCommand(uint8_t command, const uint8_t* data, uint8_t dataLen) {
...
bool LanfengProtocol::waitForResponse(uint8_t expectedCmd, unsigned long timeout) {
...
bool LanfengProtocol::parsePacket() {
...
uint8_t LanfengProtocol::calculateChecksum(const uint8_t* buffer, uint8_t length) {
  uint8_t checksum = 0;
...
void LanfengProtocol::debugPrint(const char* msg) {
  if (_debugEnabled) {
    Serial.println(msg);
  }
}

void LanfengProtocol::debugPrintHex(const char* msg, const uint8_t* data, uint8_t dataLen) {
